...
#define SK_BYTES   JADE_KEM_mlkem_kaiburr8_amd64_avx2_SECRETKEYBYTES
#define PK_BYTES   JADE_KEM_mlkem_kaiburr8_amd64_avx2_PUBLICKEYBYTES
#define CT_BYTES   JADE_KEM_mlkem_kaiburr8_amd64_avx2_CIPHERTEXTBYTES
#define SS_BYTES   JADE_KEM_mlkem_kaiburr8_amd64_avx2_BYTES
#define KP_COINS   JADE_KEM_mlkem_kaiburr8_amd64_avx2_KEYPAIRCOINBYTES
#define ENC_COINS  JADE_KEM_mlkem_kaiburr8_amd64_avx2_ENCCOINBYTES
...
int main(int argc, char **argv)
{
  ...
  printf("kaiburr8 avx2 roundtrip test (k=24, NOISE_N=8)\n");
  ...
  for (long t = 0; t < ntests; t++)
  {
    ...
    if (jade_kem_mlkem_kaiburr8_amd64_avx2_keypair_derand(pk, sk, kp_coins) != 0) { error_fail++; continue; }
    if (jade_kem_mlkem_kaiburr8_amd64_avx2_enc_derand(ct, ss_enc, pk, enc_coins) != 0) { error_fail++; continue; }

    if (jade_kem_mlkem_kaiburr8_amd64_avx2_dec(ss_dec, ct, sk) != 0) { error_fail++; continue; }
