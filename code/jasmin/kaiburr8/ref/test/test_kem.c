...
int main(void)
{
  ...
  for (int t = 0; t < NTESTS; t++)
  {
    ...
    /* --- keygen --- */
    ret = jade_kem_mlkem_kaiburr8_amd64_ref_keypair_derand(pk, sk, kp_coins);
    if (ret != 0) { printf("test %d: keypair failed (ret=%d)\n", t, ret); fclose(urandom); return -1; }

    /* --- encapsulate --- */
    ret = jade_kem_mlkem_kaiburr8_amd64_ref_enc_derand(ct, ss_enc, pk, enc_coins);
    if (ret != 0) { printf("test %d: enc failed (ret=%d)\n", t, ret); fclose(urandom); return -1; }

    /* --- decapsulate (should succeed) --- */
    ret = jade_kem_mlkem_kaiburr8_amd64_ref_dec(ss_dec, ct, sk);
